...
// Feature flag: enable payments only when required env vars are present
export const PAYMENTS_ENABLED = Boolean(
  process.env.STRIPE_SECRET_KEY && process.env.NEXT_PUBLIC_STRIPE_PREMIUM_PRICE_ID
)

// Initialize Stripe client only when enabled
export const stripe = PAYMENTS_ENABLED
...
export class StripeService {
  static async createCheckoutSession({
    ...
    try {
      if (!PAYMENTS_ENABLED) {
        console.warn('Payments are disabled; cannot create checkout session')
        return null
      }
  ...
  static async retrieveSession(sessionId: string): Promise<Stripe.Checkout.Session | null> {
    try {
      if (!PAYMENTS_ENABLED) {
        console.warn('Payments are disabled; cannot retrieve session')
        return null
      }
  ...
  static async createCustomerPortalSession(customerId: string): Promise<Stripe.BillingPortal.Session | null> {
    try {
      if (!PAYMENTS_ENABLED) {
        console.warn('Payments are disabled; cannot create portal session')
        return null
      }
  ...
  static async getSubscriptionStatus(customerId: string): Promise<{
    ...
    try {
      if (!PAYMENTS_ENABLED) {
        console.warn('Payments are disabled; cannot get subscription status')
  ...
  static constructWebhookEvent(payload: string | Buffer, signature: string): Stripe.Event | null {
    try {
      if (!PAYMENTS_ENABLED || !STRIPE_CONFIG.webhookSecret) {
        console.warn('Payments are disabled; skipping webhook construction')
        return null
      }
